...
$resultSet3 = mysqli_query($connection,"SELECT * FROM `useraddress` WHERE UserID = '$userid' ");?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
...
$(document).ready(function(){
	$('#delete').on('click',function(){
		var ID3 = $('#AddressID').val();
		if(ID3){
					$.ajax({
					type:'POST',
					url:'deleteaa.php',
					data:{"address_id3": ID3
					},
			...
			$('#delete').html('<option value="">Choose addressID First</option>');
		}
	});
});
</script>
...
$(document).ready(function(){
	$('#AddressID').on('change',function(){
		...
		if(ID2){
			$.ajax({
				type:'POST',
				url:'getaddressdata.php',
				data:{"address_ID": ID2
				},
				success:function(html){
					$('#addressName').html(html);
				}
			});
		}else{
			$('#addressName').html('<option value="">Select Address ID first</option>');
		}
	});
});
</script>
...
<div class="single-slider slider-height2 d-flex align-items-center" data-background="assets/img/hero/category.jpg" style="background-image: url(&quot;assets/img/hero/category.jpg&quot;);">
  ...
  <form  method = "POST">
    <div class="container">
      <h2>Delete Form</h2>
      <p>First choose the addressID you want to Delete</p>
      <hr>
	  <select name="ID2" id="AddressID">
                <option value="">Select Address</option>
                <?php
                    while($rows = $resultSet3->fetch_assoc())
					{
					$AddressID= $rows['AddressID'];
					echo "<option value = '$AddressID'>$AddressID</option>";
					}
                ?>
            </select>
		<br>
		<br></br>
		<h3>This is the Original information of Address before delete</h3>
				<br>
		        <label id ="addressName"></label>
				<br>
		      			             <button type="submit" id="delete" class="btn_3">
                                        Delete address
                                    </button>
	      </form>
